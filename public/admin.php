...
function getStoragePaths(): array
{
    $projectRoot = dirname(__DIR__);
    $submissionsDir = $projectRoot . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'submissions';
    $storageFile = $submissionsDir . DIRECTORY_SEPARATOR . 'inscricoes.jsonl';
    $legacyDir = __DIR__ . DIRECTORY_SEPARATOR . 'submissions';
    $legacyFile = $legacyDir . DIRECTORY_SEPARATOR . 'inscricoes.jsonl';

    if (!is_dir($submissionsDir) && !mkdir($submissionsDir, 0777, true) && !is_dir($submissionsDir)) {
        return [$storageFile, $submissionsDir];
    }

    if (is_dir($legacyDir)) {
        $legacyFiles = scandir($legacyDir);
        if (is_array($legacyFiles)) {
            foreach ($legacyFiles as $legacyName) {
                if ($legacyName === '.' || $legacyName === '..') {
                    continue;
                }
                $from = $legacyDir . DIRECTORY_SEPARATOR . $legacyName;
                $to = $submissionsDir . DIRECTORY_SEPARATOR . $legacyName;
                if (is_file($from) && !is_file($to)) {
                    @rename($from, $to);
                }
            }
        }
    } elseif (is_file($legacyFile) && !is_file($storageFile)) {
        @rename($legacyFile, $storageFile);
    }
...
[$storageFile, $submissionsDir] = getStoragePaths();
